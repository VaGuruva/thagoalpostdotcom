...
Text Domain: league-standings
*/

/*
League Standings (Wordpress Plugin)
Copyright (C) 2013-16 Mark O'Donnell
	...
	*/
	...
	// Set up localization
		...
		function mstw_ls_load_localization( ) {
			load_plugin_textdomain( 'league-standings' );
	...
	function mstw_ls_register_taxonomy() {
		...
		$labels = array( 
					'name' 				   		   => __( 'Leagues', 'league-standings' ),
					'singular_name' 			   =>  __( 'League', 'league-standings' ),
					'search_items' 				   => __( 'Search Leagues', 'league-standings' ),
					'popular_items' 			   => null, //removes tagcloud __( 'Popular Leagues', 'league-standings' ),
					'all_items' 				   => __( 'All Leagues', 'league-standings' ),
					'parent_item' 				   => null,
					'parent_item_colon' 		   => null,
					'edit_item' 				   => __( 'Edit League', 'league-standings' ), 
					'update_item'                  => __( 'Update League', 'league-standings' ),
					'add_new_item'                 => __( 'Add New League', 'league-standings' ),
					'new_item_name'                => __( 'New League Name', 'league-standings' ),
					'separate_items_with_commas'   => __( 'Separate Leagues with commas', 'league-standings' ),
					'add_or_remove_items'          => __( 'Add or Remove Leagues', 'league-standings' ),
					'choose_from_most_used'        => __( 'Choose from the most used Leagues', 'league-standings' ),
					'not_found'                    => __( 'No Leagues found', 'league-standings' ),
					'menu_name'                    => __( 'Leagues', 'league-standings' ),
				  ); 
	...
	function mstw_ls_register_post_type( ) {
		...
		$args = array(
			...
			'labels' => array(
				'name' => __( 'League Standings', 'league-standings' ),
				'singular_name' => __( 'Team', 'league-standings' ),
				'all_items' => __( 'All Teams', 'league-standings' ),
				'add_new' => __( 'Add New Team', 'league-standings' ),
				'add_new_item' => __( 'Add Team', 'league-standings' ),
				'edit_item' => __( 'Edit Team', 'league-standings' ),
				'new_item' => __( 'New Team', 'league-standings' ),
				//'View Game Schedule' needs a custom page template that is of no value.
				'view_item' => null, 
				'search_items' => __( 'Search Teams', 'league-standings' ),
				'not_found' => __( 'No Teams Found', 'league-standings' ),
				'not_found_in_trash' => __( 'No Teams Found In Trash', 'league-standings' ),
				),
	...
	function mstw_ls_shortcode_handler( $atts ){
		...
		$rank_label = mstw_set_atts_default( $options, 'ls_rank_label', __( 'Rank', 'league-standings' ) );
		$team_format = mstw_set_atts_default( $options, 'ls_team_format', 'team' ); //team|mascot|both
		$team_label = mstw_set_atts_default( $options, 'ls_team_label', __( 'Team', 'league-standings' ) );
		...
		$games_played_label =  mstw_set_atts_default( $options, 'ls_games_played_label', __( 'GP', 'league-standings' ) );
		$show_wins = mstw_set_atts_default( $options, 'ls_show_wins', 0 );
		$wins_label =  mstw_set_atts_default( $options, 'ls_wins_label', __( 'W', 'league-standings' ) );
		$show_losses = mstw_set_atts_default( $options, 'ls_show_losses', 0 );
		$losses_label =  mstw_set_atts_default( $options, 'ls_losses_label', __( 'L', 'league-standings' ) );
		$show_ties = mstw_set_atts_default( $options, 'ls_show_ties', 0 );
		$ties_label =  mstw_set_atts_default( $options, 'ls_ties_label', __( 'T', 'league-standings' ) );
		$show_other = mstw_set_atts_default( $options, 'ls_show_other', 0 );
		$other_label =  mstw_set_atts_default( $options, 'ls_other_label', __( 'OTW', 'league-standings' ) );
		$show_percent = mstw_set_atts_default( $options, 'ls_show_percent', 0 );
		$percent_label =  mstw_set_atts_default( $options, 'ls_percent_label', __( 'Percent', 'league-standings' ) );
		$show_points = mstw_set_atts_default( $options, 'ls_show_points', 0 );
		$points_label =  mstw_set_atts_default( $options, 'ls_points_label', __( 'PTS', 'league-standings' ) );
		$show_games_behind = mstw_set_atts_default( $options, 'ls_show_games_behind', 0 );
		$games_behind_label =  mstw_set_atts_default( $options, 'ls_games_behind_label', __( 'GB', 'league-standings' ) );
		$show_goals_for = mstw_set_atts_default( $options, 'ls_show_goals_for', 0 );
		$goals_for_label =  mstw_set_atts_default( $options, 'ls_goals_for_label', __( 'GF', 'league-standings' ) );
		$show_goals_against = mstw_set_atts_default( $options, 'ls_show_goals_against', 0 );
		$goals_against_label =  mstw_set_atts_default( $options, 'ls_goals_against_label', __( 'GA', 'league-standings' ) );
		$show_goals_diff = mstw_set_atts_default( $options, 'ls_show_goals_diff', 0 );
		$goals_diff_label =  mstw_set_atts_default( $options, 'ls_goals_diff_label', __( 'GD', 'league-standings' ) );
		$show_last_10 = mstw_set_atts_default( $options, 'ls_show_last_10', 0 );
		$last_10_label =  mstw_set_atts_default( $options, 'ls_last_10_label', __( 'L10', 'league-standings' ) );
		$show_last_5 = mstw_set_atts_default( $options, 'ls_show_last_5', 0 );
		$last_5_label =  mstw_set_atts_default( $options, 'ls_last_5_label', __( 'L5', 'league-standings' ) );
		$show_streak = mstw_set_atts_default( $options, 'ls_show_streak', 0 );
		$streak_label =  mstw_set_atts_default( $options, 'ls_streak_label', __( 'Streak', 'league-standings' ) );
		$show_home = mstw_set_atts_default( $options, 'ls_show_home', 0 );
		$home_label =  mstw_set_atts_default( $options, 'ls_home_label', __( 'Home', 'league-standings' ) );
		$show_away = mstw_set_atts_default( $options, 'ls_show_away', 0 );
		$away_label =  mstw_set_atts_default( $options, 'ls_away_label', __( 'Away', 'league-standings' ) );
		$show_division = mstw_set_atts_default( $options, 'ls_show_division', 0 );
		$division_label =  mstw_set_atts_default( $options, 'ls_division_label', __( 'Div', 'league-standings' ) );
		$show_conference = mstw_set_atts_default( $options, 'ls_show_conference', 0 );
		$conference_label =  mstw_set_atts_default( $options, 'ls_conference_label', __( 'Conf', 'league-standings' ) );
	...
	function mstw_ls_build_standings( $attribs ) {
		...
		if ( $teams ) {
			...
			$output =  '<h3>' . __( 'Sorry, no teams were found in league ', 'league-standings' ) . $league_id . '.</h3>';
		}
		return $output;
	...
	function widget( $args, $instance ) {
		...
		if($posts) {
        	...
        	<thead class="mstw-ls-sw-tab-head"><tr>
            	<th><?php _e( 'DATE', 'league-standings' ); ?></th>
            	<th><?php _e( 'OPPONENT', 'league-standings' ); ?></th>	
			</tr></thead>
			...
			_e( 'No Scheduled Games Found', 'league-standings' );
