...
abstract class Football_Data_Widget extends WP_Widget {
	/**
	 * Widget settings
	 * 
	...
	public function __construct( $classname, $widgetname, $description ) {
		...
		$description = __( 'Football data plugin', FOOTBALL_TEXT_DOMAIN ) . ' &raquo; ' . __( $description, FOOTBALL_TEXT_DOMAIN );
	...
	public function widget( $args, $instance ) {
		...
		if ( isset( $instance['title'] ) )
			$title = apply_filters( 'widget_title', __( $instance['title'], FOOTBALL_TEXT_DOMAIN ), $instance, $this->id_base );
		else
			$title = '';
	...
	public function widget_html( $title, $args, $instance ) {
		echo '<div class="football-data-widget">';
	...
	public function form( $instance ) {
		...
		if ( $this->widget['fields'][0]['name'] == 'Title' )
			$this->widget['fields'][0]['std'] = __( $this->widget['fields'][0]['std'], FOOTBALL_TEXT_DOMAIN );
		...
		foreach ( $this->widget['fields'] as $field ) {
			...
			if ( isset( $field['name'] ) && $field['name'] ) echo __( $field['name'], FOOTBALL_TEXT_DOMAIN ), ': ';
