...
class Ttcms_Content_Add_Push_List extends WP_List_Table
{
	/**
...
//		$this->process_actions();
	...
	public function get_views()
	{
		...
		global $wpdb;
		$results = $wpdb->get_results("SELECT * FROM feeds");
	...
	{
		 $is_active = ($item->is_active) ? 'checked' : '';
		return sprintf(
	...
	protected function column_actions($item)
	{
		...
		$actions[] = sprintf($link,
			add_query_arg('id', $item->id, add_query_arg('action', 'get-xml-data', $base_url)),
			'<span class="omb-icon-download2"></span>',
			'button icon-button',
			__('View Batch File'));

		// View Transactions
		$actions[] = sprintf($link,
			add_query_arg('id', $item->id, add_query_arg('page', 'ttcms-pf-feed-edit', admin_url('admin.php'))),
			'<span class="omb-icon-dollar"></span>',
			'button icon-button',
			__('View Batch Transactions'));
